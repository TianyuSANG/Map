...
int main()
{
    ...
    Bfs bfs;
    BfsInit(&bfs, MAP_WIDTH, MAP_HEIGHT, GetTerain);
    Pos path[1000];
    int stepNum = 0;
    Pos begin = {0, 0};
    Pos end = {19, 19};
    LOGC(LOG_LEVEL_INFO, "BfsGetPath Start");
    BfsGetPath(&bfs, begin, end, path, &stepNum);
    LOGC(LOG_LEVEL_INFO, "BfsGetPath End stepNum=%d", stepNum);
